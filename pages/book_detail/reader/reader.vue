	...
	<view>
		<view  class="reader-container">
			<view :class="'content' + (loaded ? ' loaded' : ' ') "
				:style="'height:'+windows.windows_height-1+'rpx;'+'width:' + (maxPageNum*100) +'%;' + ' -webkit-column-count:'+ maxPageNum+ ';' + 'column-count:'+ maxPageNum + ';' + 'left:' + leftValue +'rpx;' + 'background-color:' + (isUseBrightModel== 0?colorStyle.content_bg: 'transparent') + ';font-family:' +  currentFontFamily+ ';font-size:'+ allSliderValue.font+'rpx;'"
				@touchstart="handletouchtart" @touchmove="handletouchmove" @touchend="handletouchend">
				<view :style="'line-height:'+lineHeight +'rpx;'">
...
<script>
	...
	export default {
		...
		onLoad: function(options) {
			...
			uni.request({
				url: getOneBookReadingTime(userid,self.bookid),
				success(res) {
		...
		onShow: function() {
			...
			uni.request({
				url:GetStartReading(self.userid,self.bookid),
				fail(e) {
		...
		methods: {
			...
			countPageNum(str, fontSize, lineHeight, windowW, windowH, pixelRatio) {
			    ...
			    strArray.forEach(function(item, index) {
					...
					totalHeight += (Math.ceil(item.length / Math.floor(myWidth / fontSize)) *
