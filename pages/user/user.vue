<template>
	...
	<view class='today-container'>
	  ...
	  <view class="email">
	    ...
	    <!--滑块-->
	    <swiper class="swipe" duration="600" @change="changeEmail" :current="currentEmail">
	        <swiper-item v-for="item in emails" :key="item.id" :class="index == currentEmail? 'active': ''">
	          <view>
	            ...
	            <view class="content">
	              <button v-if="item.digest.content.button" @click="item.digest.content.button.eventHandler" :data-gotoUrl="item.digest.content.button.url">{{item.digest.content.button.text}}</button>
	  ...
	  <view class="dailyTask">
	    ...
	    <view class="task-item" v-for="(bookItem,bookindex) in tasks.allTasks" :key="bookItem.bookId">
	      <view class="bookDetail">
	        <text class="name">{{bookItem.bookName}}</text>
	        <text class="des">{{bookItem.bookDes}}</text>
	        <view class="hot">
	          热度：{{bookItem.bookHot}} W
	...
	export default {
		...
		onShow: function () {
			...
			uni.getStorage({
				...
				success(res) {
					uni.request({
						url:Api.GetContinueReadDay(res.data),
						success(res) {
					...
					uni.request({
						url:Api.getContinueTime(res.data),
						success(res) {
							if(res.data.code == 200){
					...
					uni.request({
						url:Api.gethasBookNum(res.data),
						success(res) {
							if(res.data.code == 200){
								console.log("gethasBookNum的结果是",res)
